...
def search_venues():
    ...
    # count = db.session.query(func.count(show.id)).group_by(show.artist_id).having(show.artist_id = 2).scalar()
...
def show_venue(venue_id):
    ...
    past_shows = []
    upcoming_shows = []
    all_upcoming_shows = Show.query.filter(Show.venue_id == venue_id and Show.start_time > date.now())
    all_past_shows = Show.query.filter(Show.venue_id == venue_id and Show.start_time < date.now())
...
def search_artists():
    ...
    for artist in results:
        count = db.session.query(func.count(Show.id)).group_by(Show.artist_id).having(Show.artist_id == artist.id).scalar()
        data.append({
            "id": artist.id,
            "name": artist.name,
            "num_upcoming_shows": count
...
def show_artist(artist_id):
    ...
    past_shows = []
    upcoming_shows = []
    all_upcoming_shows = Show.query.filter(Show.artist_id == artist_id and Show.start_time > date.now())
    all_past_shows = Show.query.filter(Show.artist_id == artist_id and Show.start_time < date.now())
...
def edit_venue_submission(venue_id):
    ...
    try:
        ...
        a_venue.address = form.address.data,
        ...
        a_venue.website_link = form.website_link.data,
